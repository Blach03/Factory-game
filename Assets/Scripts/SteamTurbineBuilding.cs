...
public class SteamTurbineBuilding : GridObject, IMachineWorkStateProvider
{
    ...
    private void ConsumeSteamAndGeneratePower()
    {
        ...
        // Najpierw sprawd� sie� przypisan� (AttachedNetwork)
        if (AttachedNetwork != null && AttachedNetwork.FluidType == steamResource)
        {
            ...
            if (AttachedNetwork.storedFluid >= toConsume)
            {
                ...
                return; // Znale�li�my paliwo, ko�czymy metod�
            }
        }
    ...
    private void NotifyAdjacentPipes()
    {
        ...
        for (int x = -1; x <= size.x; x++)
        {
            for (int y = -1; y <= size.y; y++)
            {
                // Interesuje nas tylko obw�d, nie wn�trze turbiny.
                if (x >= 0 && x < size.x && y >= 0 && y < size.y) continue;
    ...
    {
        // Najpierw usuwamy turbin� z siatki, potem od�wie�amy rury.
        base.OnDestroy();
