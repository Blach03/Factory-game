...
public class CameraController : MonoBehaviour
{
    ...
    public int viewDistanceChunks = 2; // Ile chunk�w wida� przed kamer�
    ...
    {
        // Przenosimy kamer� do 0,0 (zachowuj�c jej Z, zazwyczaj -10)
        transform.position = new Vector3(0, 0, transform.position.z);

        // Opcjonalnie: resetujemy zoom do domy�lnego (np. 15)
        if (cam != null) cam.orthographicSize = 15f;

        Debug.Log("Kamera powr�ci�a do punktu 0,0");
    }
    ...
    void CheckForNewChunks()
    {
        ...
        // Obliczamy w jakim chunku jest obecnie �rodek kamery
        ...
        // Sprawdzamy tylko je�li kamera przesun�a si� do innego chunka
        if (currentCoords != lastChunkCoords)
        {
            lastChunkCoords = currentCoords;

            // P�tla sprawdzaj�ca chunki w promieniu viewDistance
