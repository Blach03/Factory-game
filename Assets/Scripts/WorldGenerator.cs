...
public class WorldGenerator : MonoBehaviour
{
    ...
    [Tooltip("Im mniejsza liczba, tym wi�ksze plamy biom�w (0.005 - 0.02)")]
    ...
    public void InitializeWorld()
    {
        ...
        // --- LOSOWANIE SEEDU ---
        // Losujemy ogromne liczby, aby "przesun��" map� szumu w zupe�nie inne miejsce
    ...
    private float GetCurrentBiomeValue(float globalX, float globalY)
    {
        ...
        // Obliczamy dystans od �rodka �wiata (0,0)
    ...
    private void FillChunkWithBiomes(ChunkCoords coords)
    {
        ...
        for (int y = 0; y < chunkSize; y++)
        {
            for (int x = 0; x < chunkSize; x++)
            {
                ...
                // --- P�YNNE PRZEJ�CIE W CENTRUM ---
                // Je�li jeste�my blisko �rodka (np. w promieniu 80 kratek), 
                // sztucznie podbijamy warto�� szumu, �eby faworyzowa� traw�.
    ...
    // Wywo�uj to z CameraController, aby generowa� �wiat w locie
    ...
    private void SpawnResource(GameObject prefab, Vector2Int center, float dist, string type)
    {
        ...
        while (placed < totalTiles && edgeTiles.Count > 0 && attempts < maxAttempts)
        {
            ...
            if (GridManager.Instance.GetResourceDeposit(current) == null)
            {
                ...
                if (canPlace)
                {
                    ...
                    resourcePositions.Add(current); // Zapami�tujemy pozycj� kropki
                    placed++;
                }
        ...
        if (type == "water" || type == "oil")
        {
            GenerateOrganicDirtBlob(resourcePositions, 4); // Zwi�kszony zasi�g do 3 dla lepszego t�a
        }
    }
    ...
    private void GenerateOrganicDirtBlob(List<Vector2Int> corePositions, int radius)
    {
        ...
        foreach (Vector2Int core in corePositions)
        {
            for (int x = -radius; x <= radius; x++)
            {
                for (int y = -radius; y <= radius; y++)
                {
                    ...
                    // Je�li mie�ci si� w promieniu, dodajemy do zbioru (HashSet zapobiega duplikatom)
        ...
        // Teraz fizycznie stawiamy prefaby z zapami�tanych unikalnych pozycji
        foreach (Vector2Int pos in dirtPositions)
        {
            Vector3 worldPos = GridManager.Instance.GridToWorld(pos);

            // Dodajemy ma�� losowo�� na kraw�dziach bloba, �eby by� bardziej "poszarpany"
            // Sprawdzamy czy w danym miejscu ju� co� nie stoi (opcjonalnie dla optymalizacji)
    ...
    public void LoadSeed(float x, float y)
    {
        seedX = x;
        seedY = y;
        // Opcjonalnie wyczy�� stare kafelki, je�li istniej�
        if (groundTilemap != null) groundTilemap.ClearAllTiles();
        generatedTileChunks.Clear();
        generatedResourceChunks.Clear();
    }
